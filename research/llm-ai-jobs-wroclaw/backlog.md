...
| Medium | Monitor 6 Jul Wrocław/remote board burst | Dzisiejsza runda dołożyła nowy board-heavy klaster wokół `Grid Dynamics` i `FOTC`, potwierdziła że `EPAM` i `Travelplanet` nadal są aktywne, a jednocześnie `Tooploox Agentic Product Engineer` przeszło do `Offer expired` i `Vazco AI Technical Lead` wróciło z `CHF` do `EUR`. | Re-check next run whether the new board leads stay public, whether canonical employer pages appear for the new Wrocław/remote cluster, and whether the `Vazco` salary render stays on EUR. | Open |
...
| Medium | Monitor Primotly / Remodevs / Addepto / CDQ / Callstack cluster | A fresh Wrocław/remote batch surfaced today and may become recurring quickly. | Re-check next run whether the cards stay public, whether salary or remote wording drifts, and whether canonical employer pages appear. | Open |
...
| Medium | Monitor new remote Poland mini-cluster | `People Trust`, `Cyclad`, `SOFTWARELY`, `SQUARE ONE` and `CodiLime` surfaced together as a fresh remote-Poland batch with strong LLM/GenAI fit; `CodiLime` now looks materially clearer on `Poland · Fully Remote`, but the role family still spans mixed public URLs and sources. | Re-check next run whether the five roles stay public, whether `Cyclad` keeps the same salary band, and whether `CodiLime` keeps the current Poland signal without another URL/location flip. | Open |
| Medium | Monitor new YOUR ITEAMS / cerebre / Netguru cluster | The evening run added one strong direct-employer role (`YOUR ITEAMS`) plus two strong board-first roles (`cerebre`, `Netguru`), all clearly in-scope for production LLM systems and remote Poland. | Re-check next run whether all three roles remain public, whether canonical first-party pages appear for `cerebre` and `Netguru`, and whether `YOUR ITEAMS` keeps the current evaluation-heavy scope and salary band stable. | Open |
| Medium | Verify DEVAPO role-family dedup | `DEVAPO` now has both the earlier `AI / LLM Engineer` and the new `Python AI Engineer (GenAI)`, which look distinct today but overlap heavily in stack, employer, location family and board surface. | Re-check next run whether both `DEVAPO` URLs stay active and distinct, whether one disappears or gets renamed, and whether salary/location wording starts to converge. | Open |

## Review Notes

- 2026-07-05: nowa runda dołożyła board-heavy klaster `FOTC` / `Ciklum` /
  `Travelplanet` / `DCV` / `EPAM` / `Tooploox`; najbardziej konkretna zmiana
  statusowa to `Vazco AI Technical Lead`, które dziś renderuje salary w `CHF`,
  nie w wcześniejszym PLN-band.
...
- 2026-06-28: Evening run added direct-employer `YOUR ITEAMS Senior AI Engineer with .NET`, board-first `cerebre Senior AI Engineer`, board-first `Netguru AI Engineer` and a new `DEVAPO Python AI Engineer (GenAI)`; `Tooploox Senior AI Engineer` also gained a much richer employer-posted public detail, while `CodiLime` materially improved its Poland-location signal.
