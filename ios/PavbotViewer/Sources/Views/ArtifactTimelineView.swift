...
struct ArtifactTimelineView: View {
        ...
        GeometryReader { proxy in
            ...
            Group {
                if let manifest = store.manifest {
                    if let collection = selectedCollection(in: manifest) {
    ...
    private func selectedCollection(in manifest: PavbotManifest) -> PavbotArtifactCollection? {
        manifest.artifactCollection(for: router.selectedArtifactAutomationID)
            ?? manifest.artifactCollection(for: router.artifactRoute)
    }

    private func selectGroup(_ collection: PavbotArtifactCollection) {
        searchText = ""
        expandedDays = []
        let day = collection.latestArtifact?.date ?? collection.days.first?.pavbotDayString
        if let day {
            expandedDays.insert(day)
        }
        selectArtifactAutomation(id: collection.id, day: day)
    }
    ...
    private func clearFilters() {
        let currentCollection = store.manifest.flatMap { selectedCollection(in: $0) }
        let selectedDay = router.selectedArtifactDay ?? currentCollection?.latestArtifact?.date ?? currentCollection?.days.first?.pavbotDayString
        ...
        if let currentCollection {
            ...
            selectArtifactAutomation(id: currentCollection.id, day: selectedDay)
        }
    }
    ...
    private func refreshArtifacts() async {
        ...
        guard
            ...
            let collection = selectedCollection(in: manifest)
        else {
            return
        }

        let collectionIDs = Set(collection.artifacts.map(\.id))
    ...
    private func syncSelectionFromRoute() {
        ...
        guard let collection = selectedCollection(in: manifest) else {
        ...
        expandSelectedDayIfNeeded(for: collection)
    }
    ...
    private func expandSelectedDayIfNeeded(for collection: PavbotArtifactCollection) {
        if let selectedDay = router.selectedArtifactDay {
            expandedDays.insert(selectedDay)
            return
        }

        if let latestDay = collection.latestArtifact?.date ?? collection.days.first?.pavbotDayString {
...
private struct AutomationArtifactGridView: View {
    ...
    let selectAction: (PavbotArtifactCollection) -> Void
        ...
        PavbotPremiumScreenScaffold(layout: layout, spacing: 18) {
            ...
            if visibleCollections.isEmpty {
                PavbotStateCard(
                    ...
                    message: hasSearch ? "Wyczyść wyszukiwanie, aby zobaczyć wszystkie kafelki." : "Manifest nie zawiera jeszcze opublikowanych plików.",
                ...
                LazyVGrid(columns: columns, spacing: 12) {
                    ForEach(visibleCollections) { collection in
                        AutomationArtifactTile(collection: collection) {
    ...
    private var visibleCollections: [PavbotArtifactCollection] {
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
        let collections = manifest.artifactCollections
        guard !query.isEmpty else { return collections }
        return collections.filter { collectionMatchesSearch($0, query: query) }
    }
    ...
    private func collectionMatchesSearch(_ collection: PavbotArtifactCollection, query: String) -> Bool {
        ...
        return collection.artifacts.contains { $0.matchesSearch(query) }
    }
}

private struct AutomationArtifactsDetailView: View {
    @Environment(\.pavbotAdaptiveLayout) private var layout
    let collection: PavbotArtifactCollection
    ...
    var body: some View {
        PavbotPremiumScreenScaffold(layout: layout, spacing: 16) {
                ...
                AutomationArtifactsHeader(collection: collection)
                    ...
                    VStack(spacing: 10) {
                        ForEach(visibleDays, id: \.self) { day in
                            ...
                            ArtifactDayDisclosure(
                                day: day,
                                artifacts: artifacts,
                                podcastPackage: collection.automation?.kind == .podcast ? collection.podcastPackage(on: day, matching: route) : nil,
    ...
    private var visibleDays: [Date] {
        collection.days.filter { !visibleArtifacts(on: $0).isEmpty }
    }

    private var visibleOtherArtifacts: [PavbotArtifact] {
        guard route?.date == nil else { return [] }

        var artifacts = collection.otherArtifacts
    ...
    private func visibleArtifacts(on day: Date) -> [PavbotArtifact] {
        ...
        var artifacts = collection.artifacts(on: day, matching: route)
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
        if !query.isEmpty {
...
private extension PavbotArtifactCollection {
    var displayLabel: String {
        automation?.kind.label ?? "Pliki topicu"
    }

    var systemImage: String {
        automation?.kind.systemImage ?? "folder.badge.gearshape"
    }

    var statusSystemImage: String {
        isAutomationBacked ? "checkmark.circle.fill" : "folder.fill"
    }

    var tint: Color {
        automation?.kind.tint ?? .blue
    }
}
        ...
        Button(action: action) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .top) {
                    Image(systemName: collection.systemImage)
                        ...
                        .background(collection.tint.opacity(0.14), in: RoundedRectangle(cornerRadius: 8))

                    Spacer()

                    StatusBadge(text: collection.displayLabel, systemImage: collection.statusSystemImage, tint: collection.tint)
                }
                ...
                VStack(alignment: .leading, spacing: 6) {
                    TileMetric(label: "Pliki", value: "\(collection.artifacts.count)")
                    TileMetric(label: "Dni", value: "\(collection.days.count)")
                    TileMetric(label: "Ostatnio", value: collection.latestArtifact?.displayDate ?? "Brak plików")
                }
            }
            ...
            .overlay(
                RoundedRectangle(cornerRadius: layout.usesDashboardLayout ? 18 : 8)
                    .stroke(collection.tint.opacity(0.16), lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    }
...
private struct AutomationArtifactsHeader: View {
    ...
    var body: some View {
        PavbotCommandHero(
            eyebrow: collection.displayLabel,
            title: collection.title,
            subtitle: layout.usesDashboardLayout
                ? collection.topicPath
                : "Dni publikacji i wygenerowane pliki z tej kolekcji.",
            systemImage: collection.systemImage,
            tint: collection.tint,
            insights: [
                PavbotInsight(title: "Pliki", value: "\(collection.artifacts.count)", systemImage: "doc.on.doc.fill", tint: .blue),
                PavbotInsight(title: "Dni", value: "\(collection.days.count)", systemImage: "calendar", tint: .green),
                PavbotInsight(title: "Ostatnio", value: collection.latestArtifact?.date ?? "-", systemImage: "clock.fill", tint: .purple),
                PavbotInsight(title: "Temat", value: collection.topicSlug, systemImage: "folder.fill", tint: collection.tint)
            ]
        )
    }
}
