...
struct PavbotManifest: Codable, Equatable {
    ...
    var artifactCollections: [PavbotArtifactCollection] {
        let automationCollections = automationArtifactGroups.map { group in
            PavbotArtifactCollection(automationGroup: group, topic: topic(for: group.automation.topic))
        }
        let assignedArtifactIDs = Set(automationCollections.flatMap { $0.artifacts.map(\.id) })
        let unmatchedArtifacts = artifacts.filter { !assignedArtifactIDs.contains($0.id) }
        let unmatchedByTopic = Dictionary(grouping: unmatchedArtifacts, by: \.topic)
        let knownTopicOrder = Dictionary(uniqueKeysWithValues: topics.enumerated().map { ($0.element.slug, $0.offset) })

        let fallbackCollections = unmatchedByTopic
            .map { topicSlug, artifacts in
                PavbotArtifactCollection(
                    topicSlug: topicSlug,
                    topic: topic(for: topicSlug),
                    artifacts: artifacts.sorted(by: PavbotArtifact.automationDisplaySort)
                )
            }
            .sorted { lhs, rhs in
                let lhsOrder = knownTopicOrder[lhs.topicSlug] ?? Int.max
                let rhsOrder = knownTopicOrder[rhs.topicSlug] ?? Int.max
        ...
        return automationCollections + fallbackCollections
    }
    ...
    func artifactCollection(for id: String?) -> PavbotArtifactCollection? {
        guard let id else { return nil }
        return artifactCollections.first { $0.id == id }
    }
    ...
    func artifactCollection(for route: ArtifactNotificationRoute?) -> PavbotArtifactCollection? {
        guard let route else { return nil }
        var bestCollection: PavbotArtifactCollection?
        var bestScore = 0

        for collection in artifactCollections {
            let score = collection.routeMatchScore(route)
            if score > bestScore {
    ...
    private func topic(for slug: String) -> PavbotTopic? {
...
struct PavbotArtifactCollection: Identifiable, Equatable, Hashable {
    let automation: PavbotAutomation?
    let topic: PavbotTopic?
    let topicSlug: String
    let artifacts: [PavbotArtifact]

    init(automationGroup: AutomationArtifactGroup, topic: PavbotTopic?) {
        automation = automationGroup.automation
        self.topic = topic
        topicSlug = automationGroup.automation.topic
        artifacts = automationGroup.artifacts
    }

    init(topicSlug: String, topic: PavbotTopic?, artifacts: [PavbotArtifact]) {
        automation = nil
        self.topic = topic
    ...
    var id: String {
        automation?.id ?? "topic.\(topicSlug)"
    }

    var title: String {
        automation?.name ?? topic?.title ?? topicSlug
    }

    var topicPath: String {
        automation?.topicPath ?? topic?.path ?? "research/\(topicSlug)"
    }

    var isAutomationBacked: Bool {
        automation != nil
    }
    ...
    func routeMatchScore(_ route: ArtifactNotificationRoute) -> Int {
        if let topic = route.topic, topic != topicSlug {
            return 0
        }
        ...
        return route.topic == topicSlug ? 1 : 0
    }
}
