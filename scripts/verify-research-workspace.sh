...
grep -q '"rawBaseUrl": "https://raw.githubusercontent.com/19paoletto10-hub/pavbot-public-data/main/"' public/pavbot-manifest.json
grep -q 'DEFAULT_MANIFEST_URL' scripts/generate_pavbot_manifest.py
...
! grep -q 'notify.paweltanski.com' ios/PavbotViewer/Sources/Services/PavbotConnectionDefaults.swift
...
grep -q 'Manifest danych' ios/PavbotViewer/Sources/Views/SettingsView.swift
...
if [[ -n "$latest_mobile_news_data" ]]; then
  python3 - public/pavbot-manifest.json "$latest_mobile_news_data" <<'PY'
import json
import sys
from pathlib import Path
...
print(
    f"manifest missing latest mobileNewsData artifact: {expected_path}",
    file=sys.stderr,
)
raise SystemExit(1)
PY
