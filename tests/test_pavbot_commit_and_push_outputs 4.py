...
class PavbotCommitAndPushOutputsTest(unittest.TestCase):
    ...
    def test_mobile_topic_isolated_publish_includes_mobile_news_data_without_topic_file(self) -> None:
        with self.temporary_repo() as repo:
            self.git(
                repo,
                "rm",
            ...
            self.git(repo, "commit", "-m", "make mobile topic public-only")
            self.git(repo, "push", "origin", "main")

            topic_root = repo / "research" / "aktualne-wydarzenia-mobile"
            data_path = topic_root / "data" / "2026-06-25-1015-mobile-news.json"
            data_path.parent.mkdir(parents=True, exist_ok=True)
            data_path.write_text(
                json.dumps(self.valid_mobile_news_data_payload(), ensure_ascii=False) + "\n",
                encoding="utf-8",
            )
            pdf_path = topic_root / "pdfs" / "2026-06-25-1015-mobile-brief.pdf"
            pdf_path.parent.mkdir(parents=True, exist_ok=True)
            pdf_path.write_text("%PDF mobile brief", encoding="utf-8")
            script_path = topic_root / "podcasts" / "2026-06-25-1015" / "script.md"
            script_path.parent.mkdir(parents=True, exist_ok=True)
            script_path.write_text("# Podcast script\n\nTekst do lokalnego TTS.\n", encoding="utf-8")
    ...
    def test_mobile_topic_isolated_publish_refreshes_stale_manifest_without_file_changes(self) -> None:
        with self.temporary_repo() as repo:
            self.git(
                repo,
                "rm",
            ...
            self.git(repo, "add", ".")
            self.git(repo, "commit", "-m", "seed stale public mobile outputs")
            self.git(repo, "push", "origin", "main")
...
""",
    ...
    def run_publish_script(
        ...
        env.pop("PAVBOT_CLOUDKIT_DRY_RUN", None)
        if manifest_url is None:
            env.pop("PAVBOT_MANIFEST_URL", None)
        else:
            env["PAVBOT_MANIFEST_URL"] = manifest_url
        cloudkit_stub = repo.parent / "fake_cloudkit_publisher.sh"
        cloudkit_stub.write_text(
            """#!/usr/bin/env bash
set -euo pipefail
mode="${1:-}"
printf '%s\n' "$mode" >> "${PAVBOT_TEST_CLOUDKIT_LOG:?}"
case "$mode" in
  dry-run|preflight|publish|verify) exit 0 ;;
  *) exit 2 ;;
esac
""",
            encoding="utf-8",
